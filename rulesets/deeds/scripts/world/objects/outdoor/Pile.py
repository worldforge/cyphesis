...
class Pile(server.Thing):
    ...
    def sift_operation(self, op):
        if self.props._worms and self.props._worms > 0:
            ...
            imaginary_op = Operation("imaginary", Entity(description="You found a worm in the pile."), to=op.id, from_=op.id)

            return create_op, set_op, imaginary_op
        else:
            return Operation("imaginary", Entity(description="You couldn't find any worms in this pile."), to=op.id, from_=op.id)
    ...
    def dig_operation(self, op):
        ...
        if scale_prop:
            if len(scale_prop) == 3:
                set_ent.scale = [scale_prop[0] - scale_reduction, scale_prop[1] - scale_reduction, scale_prop[2] - scale_reduction]
            else:
                set_ent.scale = [scale_prop[0] - scale_reduction]

        return Operation("set", set_ent, to=self), Operation("imaginary", Entity(description="You dispersed the pile."), to=op.id, from_=op.id)
