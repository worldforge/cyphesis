...
Steering::Steering(Awareness& awareness, MemEntity& avatar) :
...
bool Steering::updatePath()
{
	...
	int result = mAwareness.findPath(mAvatar.m_location.m_pos, mViewDestination, mPath);
	EventPathUpdated();
	mUpdateNeeded = false;
	return result > 0;
}
			...
			if (WFMath::Distance(WFMath::Point<2>(finalDestination.x(), finalDestination.y()), entityPosition) < 0.1f) {
				...
				if (shouldSend) {
					...
					if (destination.isValid() && !avoiding) {
					    ...
					    WFMath::Vector<3> vel = result.destination - mAvatar.m_location.m_pos;
