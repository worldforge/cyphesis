...
class PlayerControlled(server.Thing):
                ...
                if op.refno == self.tick_refno:
                    if hasattr(arg, "type ") and arg.type == "remove":
                        ...
                        if limbo_entity and self.location.parent != limbo_entity:
                            ...
                            res += Operation("set", Entity(self.id, __respawn={"loc": self.location.parent.id, "pos": self.location.pos}), to=self.id)
    ...
    def delete_operation(self, op):
        ...
        if limbo_entity and self.location.parent != limbo_entity:
            ...
            return server.OPERATION_BLOCKED, imaginary_op, Operation("move", Entity(self.id, loc=limbo_entity.id), to=self.id), set_op, tick_op
